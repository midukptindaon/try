...
class face_mesh_detector():
    def __init__(self, static_mode = False, maxFaces = 2, refine_landmarks = False, minDetectionCon= 0.5, minTrackCon = 0.5):
        ...
        self.FaceMesh = mp.solutions.face_mesh.FaceMesh(self.static_mode,self.maxFaces,self.refine_landmarks, self.minDetectionCon,self.minTrackCon)
    ...
    def findFacesMesh(self, img, draw=True):
        ...
        if self.results.multi_face_landmarks:
            ...
            for faceLms in self.results.multi_face_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(faceLms, self.mpFaceMesh.FACEMESH_CONTOURS,self.drawSpec)
...
def main():
    ...
    while True:
        ...
        fps = 1 / (cTime-pTime)
        pTime = cTime
        cv2.putText(img, f'FrPs:{int(fps)}', (20, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
        cv2\
            .imshow("image", img)
        cv2.waitKey(1)
